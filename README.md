...
3) Apply **all** (`EnumExtender.dll, AutoUpdate.dll, ConfigMachine.dll, CustomAssets.dll `(if you want custom music)` and CustomRegions.dll`) the mods inside the `[DOWNLOAD_THIS_Custom-Regions-vX.X.zip]` file. You will receive updates automatically.
